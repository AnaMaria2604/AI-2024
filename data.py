...
headers = [cell.value for cell in sheet[
    1]]  # luam header-urile pentru a putea in dictionarele din lista data sa putem pune header(key) : valoare(value)
...
for i in data:
    try:
        ...
        print(
            f"Eroare: {e} pentru: {i}. Vom elimina acest duplicat.")  # dorim sa scoatem in evidenta duplicatele printr-o eroare care nu opreste rularea
...
for key in none_dict:
    print(f"    {i}. S-au gasit casete goale pe coloana cu numele: '{key}' de {none_dict[key]} ori.")
    i+=1
