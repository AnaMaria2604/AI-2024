...
def distribution_and_extremes(correlation_matrix):
    ...
    intervals = np.arange(-1, 1.1, 0.1)  # [-1, -0.9], [-0.9, -0.8], ..., [0.9, 1.0]
    
    # initializam dictionar cu cheile (intervalele) si 0-uri
    count_per_interval = {}
    for i in range(len(intervals) - 1):
        key = f"[{round(intervals[i], 1)}, {round(intervals[i+1], 1)})" # cheile dictionarului sunt sub forma [start, end)
   ...
   # calculam numarul de valori din fiecare interval
    for i in range(len(intervals) - 1):
        ...
        count_per_interval[key] = np.sum((matrix_values >= start_interval) & (matrix_values < end_interval))

    # print
    for interval, count in count_per_interval.items():
        print(f"{interval}: {count} valori")
    
    print(np.sum(matrix_values == 1.0), "valori sunt egale cu 1") # sunt 27 valori, avem 27 de coloane -> avem 1 doar pe diagonala principala
